# 📖 Подробное руководство по устройству проекта ReportSystem (Финалая версия)

Этот документ написан простым языком для людей, которые не занимались разработкой этой программы. Он объясняет, как устроено приложение, какие технологии в нём используются и как они взаимодействуют друг с другом шаг за шагом в его финальной чистой версии.
...
* 📁 **`ReportSystem`** (Корневая папка проекта)
  * 📁 **`Models`** — **Модели данных**. Здесь лежат C#-классы, которые описывают структуру таблиц в базе данных. Они полностью очищены от лишних комментариев.
  ...
  * 📁 **`Data`** — **Слой данных**.
    * 📄 [ApplicationDbContext.cs](file:///c:/Projects/ReportsSystem/Data/ApplicationDbContext.cs) — Самый важный файл для работы с базой данных. Здесь прописана строка подключения к PostgreSQL, настроены связи между таблицами и заданы начальные данные (роли, статусы, категории и администратор по умолчанию).
  ...
  * 📄 [App.axaml](file:///c:/Projects/ReportsSystem/App.axaml) / [App.axaml.cs](file:///c:/Projects/ReportsSystem/App.axaml.cs) — Конфигурация приложения. Именно здесь указывается, что первым делом при запуске должно открываться окно входа, а также подключаются глобальные стили и библиотеки отображения (например, стили для `DataGrid`).
...
База данных — это наборы взаимосвязанных таблиц. В нашем проекте используется PostgreSQL. Связи настроены в файле [ApplicationDbContext.cs](file:///c:/Projects/ReportsSystem/Data/ApplicationDbContext.cs).
...
> Чтобы база данных понимала, кто есть кто, в файле [ApplicationDbContext.cs](file:///c:/Projects/ReportsSystem/Data/ApplicationDbContext.cs) эти связи прописаны вручную через специальный инструмент **Fluent API** (строки 24-32).

> [!TIP]
> **Инициализация дефолтного администратора:**
> На уровне базы данных настроено автоматическое создание учетной записи администратора в методе `OnModelCreating` (строки 64-75). Поэтому сразу при развертывании приложения в системе существует аккаунт с логином `admin` и паролем `admin`, который позволяет войти и администрировать систему без ручных манипуляций в СУБД.
...
1. Пользователь вводит в поля ввода логин (например, `admin`) и пароль (`admin`) и нажимает кнопку **«Войти»**.
...
4. **Проверка безопасности:** Пароли в базе данных хранятся в виде нечитаемых строк (хэшей), например: `$2a$11$abc123xyz...`. Программа берёт введённый пароль `admin` и отправляет его в библиотеку **BCrypt**:
   ```csharp
   BCrypt.Net.BCrypt.Verify(Password, user.PasswordHash)
   ```
   *BCrypt специальным образом преобразует введенный пароль и сравнивает его с хэшем из БД. Если они соответствуют друг другу, проверка пройдена.*
...
3. В файле разметки [MainWindow.axaml](file:///c:/Projects/ReportsSystem/Views/MainWindow.axaml) кнопки бокового меню сгруппированы в панели, видимость которых жестко привязана к ролям:
   ...
   <StackPanel IsVisible="{Binding IsStudent}">
       <Button Name="NavDashboard" Content="Главная" />
       <Button Name="NavCreate" Content="Создать жалобу" />
       <Button Name="NavMy" Content="Мои обращения" />
   </StackPanel>
   ```
...
2. Откройте терминал в папке проекта и выполните команду для создания таблиц:
   ...
   ```
   *EF Core автоматически создаст все таблицы и заполнит их начальными ролями, статусами, категориями жалоб и дефолтным админом.*
