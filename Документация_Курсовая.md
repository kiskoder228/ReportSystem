...
* 📁 **`ReportSystem`** (Корневая папка проекта)
  * 📁 **`Models`** — **Модели данных**. C#-классы, описывающие структуру таблиц в БД.
    * 📄 [User.cs](file:///c:/Projects/ReportsSystem/Models/User.cs) — Пользователь (ФИО, логин, хэш пароля, роль, почта).
    ...
    * 📄 [Evidence.cs](file:///c:/Projects/ReportsSystem/Models/Evidence.cs) — Заготовка под файлы-доказательства.
...
1. **Таблица доказательств (`Evidences`):** Спроектирована в БД, но UI для загрузки файлов пока не реализован.
   - *Ответ на защите:* «Таблица создана и готова к хранению ссылок на файлы. Графическая панель загрузки запланирована на следующую итерацию».
